...
class Agent:
    ...
    def update_network_parameters(self, tau=None):
        ...
        crictic_1_state_dict = dict(critic_1_params)
        crictic_2_state_dict = dict(critic_2_params)
        target_actor_state_dict = dict(target_actor_params)
        target_critic_1_state_dict = dict(target_critic_1_params)
        target_critic_2_state_dict = dict(target_critic_2_params)

        for name in crictic_1_state_dict:
            crictic_1_state_dict[name] = tau*crictic_1_state_dict[name].clone() + (1-tau)*target_critic_1_state_dict[name].clone()

        for name in crictic_2_state_dict:
            crictic_2_state_dict[name] = tau*crictic_2_state_dict[name].clone() + (1-tau)*target_critic_2_state_dict[name].clone()

        for name in actor_state_dict:
            actor_state_dict[name] = tau*actor_state_dict[name].clone() + (1-tau)*target_actor_state_dict[name].clone()

        self.target_critic_1.load_state_dict(crictic_1_state_dict)
        self.target_critic_2.load_state_dict(crictic_2_state_dict)
